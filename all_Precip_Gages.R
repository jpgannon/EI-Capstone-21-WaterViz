...
well3_Precip <- read_csv("wxsta1_Wx_1_rain.dat",
                         skip = 4, col_names = c(X1 = "TIMESTAMP" , X2 = "RECORD", X3 ="GageMinV", 
                                                 X4= "ActTemp" , X5= "ActDepth", X6= "ReportPCP", 
                                                 X7= "ODPCounts", X8= "blockedSec", X9=  "Scan10" , 
                                                 X10= "ActDepthRA")) %>% 
...
well3_PrecipF <- filter(well3_Precip, as.Date(well3_Precip$TIMESTAMP) >= "2020-12-14")
...
ggplot(data= well3_PrecipF, aes(x= TIMESTAMP, y= ReportPCP)) +
  geom_bar(stat= "identity") +
  xlab("Date")+
  ylab("Amount of Precipitation (cm)")+
  ggtitle("Precipitation Data: Well 3")+
  theme_bw()